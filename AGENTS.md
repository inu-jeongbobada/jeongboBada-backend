...
## 코드 작성 원칙: 아직 일어나지 않은 일을 예상해서 코드를 짜지 않는다
지금 요구사항과 지금 코드에 필요한 것만 작성한다. 미래를 예상해서 넣은 코드는 대부분 쓰이지 않고, 코드만 이상하게 만든다.

- 아직 결정되지 않은 기획(예: "논의 필요" 이슈)을 짐작해서 필드·분기·설정·확장 포인트를 미리 만들지 않는다. 결정이 나면 그때 추가한다
- 호출하는 곳이 하나뿐인데 "나중에 재사용할 수도 있으니까" 추상화(인터페이스, 공용 유틸, 제네릭)를 만들지 않는다
- 응답 DTO에 "나중에 쓸 수도 있는" 필드를 null로 미리 넣지 않는다. 필드는 나중에 추가해도 기존 클라이언트가 깨지지 않는다
- 주석에도 "나중에 바뀔 수 있다", "추후 확장 예정" 같은 예측을 쓰지 않는다. 지금 코드가 **왜** 이렇게 됐는지만 쓴다
- 남은 일은 코드가 아니라 **이슈**나 `docs/{도메인}-architecture.md` 체크리스트에 적는다
